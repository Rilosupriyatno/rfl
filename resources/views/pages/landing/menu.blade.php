            ...
            <div class="bg-bgRt max-w-rattan-mobile h-[360px] mx-auto rounded-b-[32px] object-contain">
                <div class="bg-black bg-opacity-50 max-w-rattan-mobile h-[360px] mx-auto rounded-b-[32px]">
                    <div class="max-w-rattan-mobile pt-[10rem] mx-auto px-[2rem] flex items-center justify-center">
                        <img class="h-[7rem] mx-auto" src="{{ asset('assets/icons/logo-bgwhite.png') }}"
                            alt="Rattan Logo" loading="lazy" />
                    </div>
                    <div
                        class="max-w-rattan-mobile py-[10rem] mx-auto px-[2rem] flex items-center justify-center gap-[1rem]">
                        <input type="text" placeholder="Cari"
                            class="w-full border border-gray-400 rounded-[50px] px-[1.2rem] py-[0.5rem] text-[1.2rem] placeholder:text-[1.2rem] placeholder:italic placeholder:text-black placeholder:font-roboto focus:outline-none focus:border-gray-500" />
                        <a href="/dashboard/logout"
                            class="bg-[#FFCC33] flex items-center px-[2.5rem] py-[0.5rem] text-[1.2rem] font-roboto italic rounded-[0.2rem] transition-all duration-[0.4s] hover:bg-[#FFBA00]">Keluar</a>
                    </div>
                </div>
            </div>
        </header>
            ...
            <section class="flex w-full py-[1rem]">
                ...
                <div class="grid grid-flow-col auto-cols-max gap-4">
                    ...
                    <div class="w-72 rounded-xl overflow-hidden bg-white ">
                        <a href=" {{ url($data->url . $data->id) }}"
                            class="items-center justify-center gap-[0.8rem] w-[10rem] basis-[10rem] p-36">
                    ...
                    <div class="w-72 rounded-xl overflow-hidden bg-white">
                        <a href="#" class="items-center justify-center gap-[0.8rem] w-[10rem] basis-[10rem] p-36">
                            <img class="w-[7rem] mx-auto" src="{{ asset('assets/icons/orderrequest.png') }}"
...
@endif
          ...
          <a href="{{ url($data->url . $data->id) }}" class="flex flex-col items-center justify-center gap-[0.8rem] w-[10rem] basis-[10rem] py-[1rem]">
            <img src="{{ asset($data->image) }}" class="w-[4rem]" alt="chair-wicker" />
            <span class="text-center break-all text-[#767676]">{{ $data->description }}</span>
          </a>
          <a href="#" class="flex flex-col items-center justify-center gap-[0.8rem] w-[10rem] basis-[10rem] py-[1rem]">
            <img src="{{ asset('assets/icons/orderrequest.png') }}" class="w-[4rem]" alt="chair-wicker" />
            <span class="text-center break-all text-[#767676]">{{ $data->description }}</span>
          </a>
          @if ($loop->last)
</div>
...
@endforeach
        ...
        <footer class="fixed w-full bottom-0 left-0">
            ...
            <span>Whislist</span>
                    ...
                    <a {{-- href="{{ route('cart', ['index='.$parameter,'route='.$route]) }}"  --}} href="{{ route('cart') }}"
